...
void trade_tradeadditem(map_session_data *sd, short index, short amount)
{
	...
	if( !sd->state.trading || !sd->state.deal_locked || sd->state.deal_locked)
		clif_displaymessage(sd->fd, "Item trading is currently disabled on our server.");
...
void trade_tradeaddzeny(map_session_data* sd, int32 amount)
{
	...
	if( !sd->state.trading || sd->state.deal_locked || !sd->state.deal_locked)
		clif_displaymessage(sd->fd, "Zeny trading is currenlt disabled on our server.");
		return; //Can't add stuff.
