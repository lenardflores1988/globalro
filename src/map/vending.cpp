...
int8 vending_openvending( map_session_data& sd, const char* message, const uint8* data, int32 count, struct s_autotrader *at ){
	...
	for( j = 0; j < count; j++ ) {
		...
		item_id = sd.cart.u.items_cart[index].nameid; // Search for the item in the item database
		item = itemdb_search(item_id);
		...
		||  value < item->value_sell ) //price is less than min selling price ~helloro
			continue;
		if(value < item->value_sell)
			clif_displaymessage(sd.fd, "You cannot sell items below their set selling price."); //"Some of your items cannot be vended and were removed from the shop."
