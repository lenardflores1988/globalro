...
static void autopots_main(struct map_session_data* sd) {
	...
	if (pc_readreg2(sd, "@HP_BOT") == 0 || pc_readreg2(sd, "@SP_BOT") == 0)
	{
		if (pc_readreg2(sd, "@HP_BOT") == 0)
			pc_setreg2(sd, "@HP_BOT", 80);
		if (pc_readreg2(sd, "@SP_BOT") == 0)
			pc_setreg2(sd, "@SP_BOT", 50);
	}

	{//auto hp/sp pots
		unsigned short item_id_ = pc_readreg(sd, add_str("@HP_POTS"));//sta->hotkeys[36].id;
		if ((sd->battle_status.hp * 100 / int(pc_readreg2(sd, "@HP_BOT"))) < sd->battle_status.max_hp && item_id_ > 0)
		{
			i_ = pc_search_inventory(sd, item_id_);
			if (i_ >= 0)
				pc_useitem(sd, i_);
		}
		item_id_ = pc_readreg(sd, add_str("@SP_POTS"));//sta->hotkeys[37].id;
		if ((sd->battle_status.sp * 100 / int(pc_readreg2(sd, "@SP_BOT"))) < sd->battle_status.max_sp && item_id_ > 0)
		{
			i_ = pc_search_inventory(sd, item_id_);
			if (i_ >= 0)
				pc_useitem(sd, i_);
	...
	/*******Autobuffpots**********/
	for (int l_ = 27; l_ <= 35; l_++)
	{
		unsigned short item_i = sta->hotkeys[l_].id;
		//if (!sd->sc.getSCE(SC_ASPDPOTION0) && !sd->sc.getSCE(SC_ASPDPOTION1) && !sd->sc.getSCE(SC_ASPDPOTION2))
		//
		i_ = pc_search_inventory(sd, item_i);
		switch (item_i)
		{
		case 645:	//concentration pot
			if (!sd->sc.getSCE(SC_ASPDPOTION0) && i_ >= 0)// && item_i == 645)
				pc_useitem(sd, i_);
			break;
		case 656:	//awakes
			if (!sd->sc.getSCE(SC_ASPDPOTION1) && i_ >= 0)// && item_i == 656)
				pc_useitem(sd, i_);
			break;
		case 657:		//berserk
			if (!sd->sc.getSCE(SC_ASPDPOTION2) && i_ >= 0)// && item_i == 657)
				pc_useitem(sd, i_);
			break;
		case 12216:
			if (!sd->sc.getSCE(SC_INCREASEAGI) && i_ >= 0)
				pc_useitem(sd, i_);
			break;
		case 12215:
			if (!sd->sc.getSCE(SC_BLESSING) && i_ >= 0)
				pc_useitem(sd, i_);
			break;
		case 12075:
			if (!sd->sc.getSCE(SC_STRFOOD) && i_ >= 0)
				pc_useitem(sd, i_);
			break;
		case 12090:
			if (!sd->sc.getSCE(SC_AGIFOOD) && i_ >= 0)
				pc_useitem(sd, i_);
			break;
		case 12085:
			if (!sd->sc.getSCE(SC_VITFOOD) && i_ >= 0)
				pc_useitem(sd, i_);
			break;
		case 12080:
			if (!sd->sc.getSCE(SC_INTFOOD) && i_ >= 0)
				pc_useitem(sd, i_);
			break;
		case 12095:
			if (!sd->sc.getSCE(SC_DEXFOOD) && i_ >= 0)
				pc_useitem(sd, i_);
			break;
		case 12100:
			if (!sd->sc.getSCE(SC_LUKFOOD) && i_ >= 0)
				pc_useitem(sd, i_);
			break;
		default:
			break;
...
static TIMER_FUNC(autopots_timer) {
	struct map_session_data* sd = NULL;
	sd = map_id2sd(id);
	if (sd == NULL/* || pc_issit(sd)*/)
		return 0;
	//turn off autopots when died
	if (sd->sc.option & OPTION_AUTOPOTS && pc_isdead(sd)) {
		sd->sc.option &= ~OPTION_AUTOPOTS;
		clif_displaymessage(sd->fd, "Auto Potion has been deactivated.");
	}
	if (sd->sc.option & OPTION_AUTOPOTS) {
		autopots_main(sd);
		add_timer(gettick() + 200, autopots_timer, sd->bl.id, 0);
	}
	return 0;
}
...
ACMD_FUNC(autopots)
{
	...
	if (pc_isdead(sd))
	{
		clif_displaymessage(fd, "Can not @autopots if you are dead");
		return -1;
	}

	if (sd->sc.option & OPTION_AUTOPOTS) {
		sd->sc.option &= ~OPTION_AUTOPOTS;
		clif_displaymessage(fd, "Auto Potion has been deactivated.");
	}
	else {
		sd->sc.option |= OPTION_AUTOPOTS;
		add_timer(gettick() + 500, autopots_timer, sd->bl.id, 0);
		clif_displaymessage(fd, "Auto Potion activated.");
	}
	clif_changeoption(&sd->bl);
	return 0;
}
...
bool bot_check_target(struct block_list* src, unsigned int id) {
	...
	if (bl)
	{
		if (path_search(NULL, src->m, src->x, src->y, bl->x, bl->y, 1, CELL_CHKNOREACH) && distance_xy(src->x, src->y, bl->x, bl->y) < 11)
		{
...
unsigned int bot_check_target_alive(struct block_list* src, unsigned int id, int mob_id) {
	struct mob_data* md2 = NULL;
	struct status_data* status = status_get_status_data(*src);

	if (bot_check_target(src, id)) {
		return id;
	}
	...
	if (sd && pc_readreg(sd, add_str("@WALK_BOT")) == 1) {
		range = status->rhw.range;
	}
	else {
		range = 15; // Default range
	}

	// Look for target within the specified range
	for (int i_ = 0; i_ < range; i_++) {
		int target_id_ = 0;
		map_foreachinarea(buildin_autoattack_sub, src->m, src->x - i_, src->y - i_, src->x + i_, src->y + i_, BL_MOB, &target_id_, src->id);

		md2 = (struct mob_data*)map_id2bl(target_id_);
		if (target_id_ && md2 && md2->status.hp > 0 && md2->get_bosstype() == BOSSTYPE_NONE && !md2->master_id && md2->level > 1) {
			if ((mob_id == 0) || (mob_id != 0 && md2->mob_id == mob_id)) {
				pc_setreg(map_id2sd(src->id), add_str("@SP_BOT_TGID"), target_id_);
			}
			else {
				target_id_ = 0;
			}
	...
	// Recheck for nearby targets if current target is dead
	for (int i_ = 0; i_ < range; i_++) {
		int target_id_ = 0;
		map_foreachinarea(buildin_autoattack_sub, src->m, src->x - i_, src->y - i_, src->x + i_, src->y + i_, BL_MOB, &target_id_, src->id);

		md2 = (struct mob_data*)map_id2bl(target_id_);
		if (target_id_ && md2 && md2->status.hp > 0 && md2->get_bosstype() == BOSSTYPE_NONE && !md2->master_id) {
			pc_setreg(map_id2sd(src->id), add_str("@SP_BOT_TGID"), target_id_);
		}
		else {
			target_id_ = 0;
...
void autoattack_motion(struct map_session_data* sd, int mob_id) {
	struct status_data* status = &sd->base_status;
	struct mmo_charstatus* sta = &sd->status;
	...
	unsigned int starget_id_ = bot_check_target_alive(&sd->bl, pc_readreg(sd, add_str("@SP_BOT_TGID")), mob_id);
	...
	{//Buff
		for (i_ = 21; i_ <= 25; i_++)
		{
			if ((inf_ = bot_chec_khotkeys_is_skill(sd, i_)) > 0)
			{
				...
				if (inf_ == 4 || inf_ == 16)
				{
					sc_type sctype = skill_get_sc(id_);
					if (!sd->sc.getSCE(sctype))
						unit_skilluse_id(&sd->bl, sd->bl.id, id_, sta->hotkeys[i_].lv);
				}
			}
		}
	}
	...
	if (starget_id_ > 0) {
		...
		if (/*sd->state.autotrade && */pc_readreg2(sd, "@BASICATTACK_BOT") == 0 && !check_distance_bl(&sd->bl, map_id2bl(starget_id_), status->rhw.range))
			unit_walktobl(&sd->bl, map_id2bl(starget_id_), status->rhw.range, 0);

		for (int skill_ = 18; skill_ < 21; skill_++) {
			if (sta->hotkeys[skill_].type == 1 && aa_canuseskill(sd, sta->hotkeys[skill_].id, sta->hotkeys[skill_].lv)){// status_check_skilluse(&sd->bl, map_id2bl(starget_id_), 0, 0)/* && sta->ap > skill_get_ap(sta->hotkeys[skill_].id, sta->hotkeys[skill_].lv)*/) {
				switch (skill_get_inf(sta->hotkeys[skill_].id)) {
				case INF_GROUND_SKILL:
					if (!check_distance_bl(&sd->bl, map_id2bl(starget_id_), skill_get_range(sta->hotkeys[skill_].id, sta->hotkeys[skill_].lv)))
						unit_walktobl(&sd->bl, map_id2bl(starget_id_), skill_get_range(sta->hotkeys[skill_].id, sta->hotkeys[skill_].lv), 0);
					unit_skilluse_pos(&sd->bl, map_id2bl(starget_id_)->x, map_id2bl(starget_id_)->y, sta->hotkeys[skill_].id, sta->hotkeys[skill_].lv);
					break;
				case INF_ATTACK_SKILL:
					if (!check_distance_bl(&sd->bl, map_id2bl(starget_id_), skill_get_range(sta->hotkeys[skill_].id, sta->hotkeys[skill_].lv)))
						unit_walktobl(&sd->bl, map_id2bl(starget_id_), skill_get_range(sta->hotkeys[skill_].id, sta->hotkeys[skill_].lv), 0);
					unit_skilluse_id(&sd->bl, starget_id_, sta->hotkeys[skill_].id, sta->hotkeys[skill_].lv);
					break;
		...
		if (pc_readreg2(sd, "@BASICATTACK_BOT") == 0)
			unit_attack(&sd->bl, starget_id_, 1);
	}
	else {//Move

		bool flywing_ = false;

		if (bot_check_target(&sd->bl, starget_id_) == false)
		{
			if (tele_ > 7 && idle_ % 9 == 0 && idle_ > 7 && pc_readreg2(sd, "@TELE_BOT") > 0)
			{
				unsigned short item_flywing_ = pc_readreg(sd, add_str("@FLY_WING"));
				if (pc_readreg2(sd, "@TELE_BOT") == 1 && flywing_ == false)
				{
					i_ = pc_search_inventory(sd, item_flywing_);
					if (i_ >= 0)
					{
						if (sd->state.autotrade)
						{
							pc_setpos(sd, sd->mapindex, 0, 0, CLR_TELEPORT);
							pc_useitem(sd, i_);
				...
				if (pc_readreg2(sd, "@TELE_BOT") == 2 && flywing_ == false) {
					if (pc_checkskill(sd, AL_TELEPORT) > 0 && sta->sp > 20)
					{
						if (sd->state.autotrade)
						{
		...
		if (starget_id_ == 0 && (int)pc_readreg2(sd, "@WALK_BOT") == 0 && idle_ > 0)
		{//Move
			int d = 10;
			int x, y, rdir, i, max;
			max = 20;
			rdir = pc_readreg2(sd, "@DIR_BOT");
			...
			case 0://north -east
				for (i = 0; i < max; i++)
				{
					...
					do {
						...
						y = rnd() % 7 + sd->bl.y;
						j++;
					} while (!unit_walktoxy(&sd->bl, x, y, 0) && j < max);
					if (j == max) {
						int jr = rand() % 2;
						switch (jr) {
						case 0:
							pc_setreg2(sd, "@DIR_BOT", 1);
							break;
						case 1:
							pc_setreg2(sd, "@DIR_BOT", 3);
							break;
						}
						break;
					}
			...
			case 1:
				for (i = 0; i < max; i++)
				{
					...
					if (j == max) {
						...
						switch (jr) {
						case 0:
							pc_setreg2(sd, "@DIR_BOT", 0);
							break;
						case 1:
							pc_setreg2(sd, "@DIR_BOT", 2);
							break;
						}
						break;
					}
...
static TIMER_FUNC(autoattack_timer) {
	...
	if (sd->state.autoattack) {
		...
		add_timer(gettick() + 500, autoattack_timer, sd->bl.id, data);
	}
	return 0;
}
...
ACMD_FUNC(autoattack) {
	...
	if (!message || !*message) {
		...
		clif_displaymessage(fd, "@autoattack reset - Remove all target monsters");
	...
	if (strcmp(message, "off") == 0) {
		...
		//sd->state.block_action &= ~(PCBLOCK_NPC | PCBLOCK_NPCCLICK);
	...
	if (strcmp(message, "on") == 0) {
		...
		if (!added_monsters.empty()) {
			...
			for (int monster_id : added_monsters) {
				add_timer(gettick() + 1000, autoattack_timer, sd->bl.id, (intptr_t)monster_id);
			}
	...
	if (strcmp(message, "reset") == 0) {
		if (added_monsters.empty()) {
			clif_displaymessage(fd, "List is empty");

			return -1;
		}
		else {
			added_monsters.clear();
			//added_monsters.
			return 0;
		}
	}
...
ACMD_FUNC(afk)
{
	...
	{
		clif_displaymessage(fd, "Cannot @afk if you are dead/sitting.");
		return -1;
	}
	sd->state.autotrade = 1;
	clif_changelook(&sd->bl, LOOK_HEAD_BOTTOM, 471);;
	sc->cant.warp = 1;
